...
export const fetchQuestionAttempt = async (quiz_attempt_id: string, question_number: number): Promise<any> => {
  try {
    // The second argument to fetch is the options object
    const token = localStorage.getItem(ACCESS_TOKEN);
    const baseURL = import.meta.env.VITE_API_URL
    const url = `${baseURL}/api/quiz_attempts/${quiz_attempt_id}/create_next_question_attempt/`;
    console.log("*** fetchQuestionAttempt url=", url, ' question_number=', question_number);
    //path("quiz_attempts/<int:pk>/create_next_question_attempt/", views.create_question_attempt),  # pk i
    const response = await fetch(url, {
      method: 'POST', // Or 'POST', 'PUT', 'DELETE', etc.
      body: JSON.stringify({ question_number: question_number }), // Send question_id in the request body
      headers: {
